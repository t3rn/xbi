...
impl Assets {
    ...
    pub fn len(&self) -> usize {
        self.fungible
            .len()
            .checked_add(self.non_fungible.len())
            .unwrap()
    }
    ...
    pub fn fungible_assets_iter<'a>(&'a self) -> impl Iterator<Item = MultiAsset> + 'a {
        self.fungible.iter().map(|(id, &amount)| MultiAsset {
            fun: Fungible(amount),
            id: id.clone(),
        })
    }
    ...
    pub fn non_fungible_assets_iter<'a>(&'a self) -> impl Iterator<Item = MultiAsset> + 'a {
        self.non_fungible.iter().map(|(id, instance)| MultiAsset {
    ...
    pub fn assets_iter<'a>(&'a self) -> impl Iterator<Item = MultiAsset> + 'a {
        self.fungible_assets_iter()
            .chain(self.non_fungible_assets_iter())
    }
        ...
        self.fungible = fungible
            ...
            .filter_map(|(mut id, amount)| match id.reanchor(target, ancestry) {
                ...
                Err(()) => {
                    maybe_failed_bin
                        .as_mut()
                        .map(|f| f.fungible.insert(id, amount));
                    None
                },
            })
            .collect();
        ...
        self.non_fungible = non_fungible
            ...
            .filter_map(|(mut class, inst)| match class.reanchor(target, ancestry) {
                ...
                Err(()) => {
                    maybe_failed_bin
                        .as_mut()
                        .map(|f| f.non_fungible.insert((class, inst)));
                    None
                },
            })
            .collect();
                        ...
                        MultiAsset {
                            ...
                            let (remove, amount) = match self.fungible.get_mut(&id) {
                                Some(self_amount) => {
                                    ...
                                    *self_amount = *self_amount.checked_sub(amount).unwrap();
